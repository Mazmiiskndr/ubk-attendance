...
use App\Livewire\Forms\Backend\Attendance\Lecture\UpdateDateForm;
...
class EditDate extends Component
{
    ...
    {
        return view('livewire.backend.attendance.lecture.edit-date');
    }

    /**
     * The properties of a attendances object.
     */
    #[On('deliverAttendanceLectureToEditComponent')]
    ...
    public function updateAttendanceDate(AttendanceService $attendanceService)
    {
        ...
        if ($attendance) {
            ...
            $this->dispatch('show-toast', ['type' => 'success', 'message' => 'Presensi Dosen Pertanggal berhasil di perbaharui!']);
            // Let other components know that a setting was updated
            $this->dispatch('attendanceLectureUpdated', $attendance);
        } else {
            // Notify the frontend of failure
            $this->dispatchErrorEvent('Gagal Mengubah Presensi Dosen Pertanggal');
        }
        // Close the modal
        $this->closeModal();
    }
